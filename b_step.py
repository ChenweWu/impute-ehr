...
from remasker_impute_step import ReMaskerStep
import numpy as np
import pandas as pd
...
X = df.filter(regex='^test[1-8](_yesterday)?$') 
print(X.columns.values)
X_train, X_test = train_test_split(X, test_size=0.2, random_state=42)
X_train.to_csv("X_train16.csv",index=False)
X_test.to_csv("X_test16.csv",index=False)
